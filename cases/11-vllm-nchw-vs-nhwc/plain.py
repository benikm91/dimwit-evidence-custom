"""Case 11 — NCHW frames fed to an NHWC pipeline (NumPy).

Original: vllm-project/vllm#51076.
"""
...
def decode_nchw(n_frames=2, channels=3, height=3, width=3):
    ...
    return RNG.normal(size=(n_frames, channels, height, width))


def decode_nhwc(n_frames=2, channels=3, height=3, width=3):
    """The other backend: [frames, height, width, channels]."""
    return RNG.normal(size=(n_frames, height, width, channels))
...
def to_nhwc(frames_nchw):
    ...
    return np.transpose(frames_nchw, (0, 2, 3, 1))
...
def test_per_channel_statistics_are_computed_over_the_wrong_axes():
    ...
    nchw = decode_nchw()
    wrong = per_channel_mean_nhwc(nchw)          # reduces frames, channels, rows
    right = per_channel_mean_nhwc(to_nhwc(nchw))  # reduces frames, rows, columns
...
def test_no_exception_is_raised_anywhere():
    nchw = decode_nchw()
    assert per_channel_mean_nhwc(nchw).shape == (3,)
